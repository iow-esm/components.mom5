...
#ifndef MAX_BF
#  define MAX_BF 1024 /* maximum blocking factor */
#endif
	       ...
	       if (peWidth<0)
		 {
	       ...
	       sprintf(infilename,"%s.%0*d",outfilename,peWidth,a);
              ...
              infileerror=process_file(infilename,appendnc,ncoutfile,
                                       ...
                                       headerpad,verbose,missing,defl);
...
int process_file(char *ncname, unsigned char appendnc,
                 ...
                 unsigned char verbose, unsigned char missing, int defl)
  {
   ...
   if ((!appendnc) && (!mem_dry_run))
     {
      ...
      for (v=0; v < ncinfile->nvars; v++)
        {
         ncoutfile->varmiss[v]=0;
         varID=ncvardef(ncoutfile->ncfid,ncinfile->varname[v],ncinfile->datatype[v],
